let STATIC_URL = `http://${document.domain}:${location.port}/static/`;
...
var messages = {
  ...
  $( document ).ready(function() {
    $("#start_img").attr("src", `${STATIC_URL}img/mic.gif`);
    console.log(`${STATIC_URL}img/mic.gif`);
    ...
    if (!('webkitSpeechRecognition' in window)) {
      ...
      recognition.onstart = function() {
        recognizing = true;
        showInfo('speak_now');
        start_img.src = STATIC_URL + 'img/mic-animation.gif';
      };
  
      recognition.onerror = function(event) {
        if (event.error == 'no-speech') {
          start_img.src = STATIC_URL + 'img/mic.gif';
          showInfo('no_speech');
          ignore_onend = true;
        }
  ...
  $("#start_button").click(function (event) {
    ...
    start_img.src = STATIC_URL + 'img/mic-slash.gif';
    showInfo('allow');
