...
$(document).ready(function () {
  ...
  $("#start_img").attr("src", `${IMAGES_URL}/mic.gif`);
  ...
  if (!('webkitSpeechRecognition' in window)) {
    ...
    showInfo('start');
    speech_mic.style.display = 'inline-block';
    ...
    recognition.onstart = function () {
      recognizing = true;
      showInfo('speak_now');
      start_img.src = `${IMAGES_URL}/mic-animation.gif`;
    };

    recognition.onerror = function (event) {
      if (event.error == 'no-speech') {
        start_img.src = `${IMAGES_URL}/mic.gif`;
        showInfo('no_speech');
        ignore_onend = true;
      }
    ...
    recognition.onresult = function (event) {
      ...
      $(DOMS.CHAT_INPUT).val(interim_transcript);
      if(final_transcript.trim() !== "") {
        $(DOMS.CHAT_INPUT).val(final_transcript);
      }
    };
  }
});
...
function upgrade() {
  speech_mic.style.visibility = 'hidden';
  showInfo('upgrade');
}
...
$("#speech_mic").click(function (event) {
  ...
  recognition.start();
  ignore_onend = false;
  start_img.src = `${IMAGES_URL}/mic-slash.gif`;
  showInfo('allow');
